...
ThisUserIdIU = $(this).attr('data-userlogs');
	...
	$(document).on('click','.IU-posts-btn',function(){
			...
			$('.brd-tbl-users').hide();
				...
				$.ajax({
				  ...
				  success: function(data){
						...
						$('.IU-posts-btn').addClass('active-comp-menu');				
					}
				});
				//$('.IU-container').html('123');
	});	
	...
	up();
		...
		$('.IU-user-btn').addClass('IU-active');				
			...
			ClearStartIU();
				...
				//-- StartOnload
				$.getJSON( "ajax/cusotomize/ajax/ajax_login_list_grp.php?showalllist=yes", function( data ) {
			ClearStartGrp();
					//OutUsers (data2);
					//setTimeout(function(){ 
						OutGroups(data);
...
function actstaff (staff_id,staff_inf_id){
//-default Params
valChange = $('.'+prefix+'sel-page-wrap-opt').val();
valStatus = $('#stats-staff').val();
valSearchType = $('#staff_paramsearch').val();
...
valLoca_filt = $('#'+prefix+'loca-list').val();	
	$.ajax({
		...
		data: {staff_id : staff_id,staff_inf_id : staff_inf_id},
		type: 'POST',
		success: function(){
			$.fancybox({ closeClick  : true});
			OutBlocks(1,valChange,valStatus,valSearchType,valSearchData,valComp_filt,valDep_filt,valLoca_filt);
...
$(document).on('click','.connectInUserNow',function(){
	staff_id = $(this).attr('data-idstaff');
	staff_lastname = $('#staff_cor_lastname').val();
	staff_name = $('#staff_cor_name').val();
	staff_middlename = $('#staff_cor_secondname').val();
	$.ajax({
	  type: "POST",
	  url: "itdept/stable/ajax/userlist/form_user/cor/checkifinfoline.php",
	  data: {staff_id: staff_id ,staff_lastname : staff_lastname, staff_name : staff_name, staff_middlename : staff_middlename},
	  success: function(data){
			/*if(data=='<span id="rezidconnectusrInf" class="rezgreeninfUs">Профиль привязан!</span>'){
				$('.connectInUserNow').remove();
			}*/
			if(data == 1){
				$('#rezultcontainerinf').html('<span id="rezidconnectusrInf" class="rezgreeninfUs">Профиль привязан!</span>');
				$('.connectInUserNow').prev().css({'background':'#090'});
				$('.brd-usr[data-staffid="'+staff_id+'"]').parent().parent().find('.connection-notshow-inf').addClass('greeninfshow');
				$('.brd-usr[data-staffid="'+staff_id+'"]').parent().parent().find('.connection-notshow-inf').removeClass('redinfshow');
				$('.connectInUserNow').remove();
			}else{
				$('#rezultcontainerinf').html('<span id="rezidconnectusrInf" class="rezredinfUs">Не нашел!</span>');
			}
	  }
	});
});
...
$(document).on('click','#post_createpost',function(){
    $.fancybox({
        //width: 400,
        maxWidth    : 713,
            minHeight   : 150,
            ...
            scrolling   : false,
            padding     : 0,
        href: 'ajax/postusers/form/create/buid_form.php',
        type: 'ajax'
    });
});


$(document).on('click','#cor-postusers-field',function(){
	dataDbId = $(this).attr('data-db-id');
    $.fancybox({
        //width: 400,
        maxWidth    : 713,
            minHeight   : 150,
            ...
            scrolling   : false,
            padding     : 0,
        href: 'ajax/postusers/form/correct/buid_form.php?id='+dataDbId,
        type: 'ajax'
    });
});

$(document).on('click','#del-postusers-field',function(){
	dataDbId = $(this).attr('data-db-id');
    $.fancybox({
        //width: 400,
        maxWidth    : 713,
            ...
            scrolling   : false,
            padding     : 0,
        href: 'ajax/postusers/form/delete/buid_form.php?id='+dataDbId,
        type: 'ajax'
    });
});
