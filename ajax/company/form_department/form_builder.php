...
<form class="formpadding" id="<?=$formName?>">
	<h1 class="form-header">Создать отдел</h1>
	<table>
		<?foreach($arrayForm as $f_Items){?>
			<?if($f_Items['name'] == $prefix.'compn'){?>
				<tr>
					...
					<td style="position:relative;">
						<select class="field-tpl" name="<?=$f_Items['name']?>" id="<?=$f_Items['name']?>">
							...
							<option <?if($_GET["id"] == $f_item["company_id"]){?>selected<?}?> value="<?=$f_item['company_id']?>"><?=$f_item['company_name']?></option>
							<?}?>
						</select>
		...
		<tr>
			<td class="field-cover">
				<label for="fld-create-url-dp-frm">Ссылка</label>
			</td>
			<td style="position:relative;">
				<input class="field-tpl" type="text" name="fld-create-url-dp-frm" id="fld-create-url-dp-frm" value="">
				<i style="  margin-left: 10px;color: rgb(0, 153, 0);" class="fa fa-check-circle exelfiledIU"></i>
			</td>
		</tr>	
		<tr>
			<td class="field-cover">
				<label for="fld-create-sorting-dp-frm">Сортировка</label>
			</td>
			<td style="position:relative;">
				<input class="field-tpl" type="text" name="fld-create-sorting-dp-frm" id="fld-create-sorting-dp-frm" value="500">
				<i style="  margin-left: 10px;color: rgb(0, 153, 0);" class="fa fa-check-circle exelfiledIU"></i>
			</td>
		</tr>
		<tr>
			<td class="field-cover">
				<label for="fld-create-sorting-dp-frm">Информация</label>
			</td>
			<td style="position:relative;">
				<textarea rows="8" class="field-tpl" type="text" name="fld-create-info-dp-frm" id="fld-create-info-dp-frm"><?=$allDataInf['dept_info']?></textarea>
			</td>
		</tr>
		<tr class="btnarea">
			<td style="vertical-align: bottom;" colspan="2">
				<div style="margin: 20px auto 0;">
					...
					<input class="btn green" type="submit" name="<?=$prefix?>sendform" value="Создать отдел">
