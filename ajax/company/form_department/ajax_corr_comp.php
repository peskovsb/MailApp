...
$sql = 'SELECT * FROM department LEFT JOIN company using(company_id) WHERE department.department_id = :department_id;';
...
$arrCompany = $tb->fetchAll(PDO::FETCH_ASSOC);
	...
	$sql = 'SELECT * from itdeptdepartment WHERE department_id=:id;';
	$tb = $db_infoline->connection->prepare($sql);
	$tb->execute([':id'=>$_GET['id']]);
	$allDataInf = $tb->fetch(PDO::FETCH_ASSOC);
...
<input name="dep_id" type="hidden" value="<?=$arrAll['department_id']?>">
	<h1 class="form-header">Редактировать отдел</h1>
	<table>
		<tr>
			...
			<td>
				<select class="field-tpl" name="fld-comp-<?=$prefix_form?>-cor" id="fld-comp-<?=$prefix_form?>-cor">
					<?foreach($arrCompany as $Items){?>
		...
		<tr>
			...
			<td>
				<input class="field-tpl" type="text" name="fld-dep-<?=$prefix_form?>-cor" id="fld-dep-<?=$prefix_form?>-cor" value="<?=$arrAll['department_name']?>">
			</td>
		</tr>
		<tr>
			<td class="field-cover">
				<label for="fld-dep-url-<?=$prefix_form?>-cor">Ссылка</label>
			</td>
			<td>
				<input class="field-tpl" type="text" name="fld-dep-url-<?=$prefix_form?>-cor" id="fld-dep-url-<?=$prefix_form?>-cor" value="<?=$allDataInf['staff_url_dept']?>">
			</td>
		</tr>
		<tr>
			<td class="field-cover">
				<label for="fld-dep-sorting-<?=$prefix_form?>-cor">Сортировка</label>
			</td>
			<td>
				<input class="field-tpl" type="text" name="fld-dep-sorting-<?=$prefix_form?>-cor" id="fld-dep-sorting-<?=$prefix_form?>-cor" value="<?=$allDataInf['staff_departmentsprting']?>">
			</td>
		</tr>
		<tr>
			<td class="field-cover">
				<label for="fld-dep-info-<?=$prefix_form?>-cor">Информация</label>
			</td>
			<td>
				<textarea rows="8" class="field-tpl" type="text" name="fld-dep-info-<?=$prefix_form?>-cor" id="fld-dep-info-<?=$prefix_form?>-cor"><?=$allDataInf['dept_info']?></textarea>
			</td>
		</tr>
		<tr class="btnarea">
