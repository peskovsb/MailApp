...
$sql = 'SELECT * FROM itdeptgroups WHERE gr_id = :gr_id';
$tb = $db_infoline->connection->prepare($sql);
$tb->execute(array(':gr_id'=>$_GET['id']));
$arrGrInf = $tb->fetch(PDO::FETCH_ASSOC);
...
<input name="dep_id" type="hidden" value="<?=$arrAll['gr_id']?>">
	...
	<table>
		...
		<tr>
			<td class="field-cover">
				<label for="fld-sort-<?=$prefix_form?>-cor">Сортировка</label>
			</td>
			<td>
				<input class="field-tpl" type="text" name="fld-sort-<?=$prefix_form?>-cor" id="cor-grps-sort-inp-list" value="<?=$arrGrInf['staff_groupssprting']?>">
			</td>
		</tr>
		<tr class="btnarea">
