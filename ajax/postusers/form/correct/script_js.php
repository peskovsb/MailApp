...
$('#wrapper-<?=$formBuild?>').on('submit','#<?=$formBuild?>-correct',function(){
	//--MainParams
...
<?}?>
        ...
        if(mistake == 0 ){
                ...
                $.ajax({
                    ...
                    data: dataForm,
                    type: "POST",
                    url: "<?=$jsScriptUrl?>/correct/ajax_submit.php",
                    success: function (data) {
						$('.mistake-field-callback').remove();                        
						
						$('#wrapper-<?=$formBuild?> .btn.green').after('<div class="mistake-field-callback" style="font-size:14px;background: #090;color: #fff;padding: 15px;width: 312px;margin: 15px 0 0 0px;text-align: center;">Должность отредактирована успешно</div>');
