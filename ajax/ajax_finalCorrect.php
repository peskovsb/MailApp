...
class mainpage_query{
	...
	function getAllfromtable($user_id,$login,$password,$email,$username,$sername,$mailbox,$userdate,$domain_id,$active,$profile_create){
		...
		if($profile_create != 0){
			...
			$sql = 'UPDATE infoline_users SET email=:email, login=:login WHERE id=:user_id';
			$tb = $db_infoline->connection->prepare($sql);
			$tb->execute(array(':user_id'=> $userData['staff_profile_infoline'],':email'=>$email,':login'=>$login));
		}		
