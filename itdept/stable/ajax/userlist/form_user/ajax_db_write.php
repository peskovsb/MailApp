...
$length = 8;
$chartypes = "all";
$stringPass = random_string($length, $chartypes);
...
$returnEntr = mktime(0,0,0,$dt_st[1], $dt_st[0], $dt_st[2]);
$returnCur = mktime(0,0,0,date('m'), date('d'), date('Y'));
if($returnEntr<=$returnCur){
	$word_change = 'вышел';
	$already_work = '1';
	
	$tob  = $emails.', infoline@bioline.ru'; 
	...
	<body>
		...
		<p style=" font-family:\'Segoe UI\';font-size: 14px;">Здравствуйте, '.$user_name.'!<br>
...
$sql = 'INSERT INTO infoline_users (`login`,`password`,`email`,`lastname`,`firstname`,`middlename`,`staff_company_id`,`staff_depart_id`,`staff_group_id`,`staff_active`,`cb_post`,`staff_location`,`cb_birthday1`,`staff_notshow`) VALUES (:login,"7777",:email,:lastname,:firstname,:middlename,:staff_company_id,:staff_depart_id,:staff_group_id,:staff_active,:cb_post,:staff_location,:cb_birthday1,:staff_notshow)';
$tb = $db_infoline->connection->prepare($sql);
$tb->execute(array(':login'=>$login,':email'=>$emails,':lastname'=>$user_last_name,':firstname'=>$user_name,':middlename'=>$user_secondname,':staff_company_id'=>$company,':staff_depart_id'=>$department,':staff_group_id'=>$groupdep,':staff_active'=>$already_work,':cb_post'=>$post_user,':staff_location'=>$location,':cb_birthday1'=>$userbdate,':staff_notshow'=>$showInfUser));
$LastInsertedInf = $db_infoline->connection->lastInsertId('id');
	
	
	$sql = 'INSERT INTO staff (`staff_name`,`staff_secondname`,`staff_lastname`,`staff_company_id`,`staff_group_id`,`staff_depart_id`,`staff_active`,`staff_formsignd`,`staff_enterdate`,`staff_post`,`staff_dopcomp1`,`staff_dopcomp2`,`staff_location`,`staff_dr`,`staff_profile_infoline`) VALUES (:staff_name,:staff_secondname,:staff_lastname,:staff_company_id,:staff_group_id,:staff_depart_id,:staff_active,now(),:staff_enterdate,:staff_post,:staff_dopcomp1,:staff_dopcomp2,:staff_location,:staff_dr,:staff_profile_infoline)';
	$tb = $db->connection->prepare($sql);
	//$db->connection->beginTransaction();	
	$tb->execute(array(':staff_name'=>$user_name,':staff_secondname'=>$user_secondname,':staff_lastname'=>$user_last_name,':staff_company_id'=>$company,':staff_group_id'=>$groupdep,':staff_depart_id'=>$department,':staff_active'=>$already_work,':staff_enterdate'=>$dateenter,':staff_post'=>$post_user,':staff_dopcomp1'=>$dop_comp1,':staff_dopcomp2'=>$dop_comp2,':staff_location'=>$location,':staff_dr'=>$userbdate,':staff_profile_infoline'=>$LastInsertedInf));
...
//print_r($data);
/*foreach($data as $ItemsInf){
	echo $ItemsInf['firstname'].'<br>';	
}*/	
...
'.$dopinfo_text.'
	...
	mail($_POST['mailsDeliver'], $subject, $messageIT, $headers);
