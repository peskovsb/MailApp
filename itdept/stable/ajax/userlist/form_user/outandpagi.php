	  ...
	  success: function(data){
		  if(data.length>0){
			...
			$.each( data, function( keying, val ) {
			 ...
			 if(val.staff_profile_infoline=='0'){colIndShow = 'redinfshow';}else{colIndShow = 'greeninfshow';}
			 ...
			 if(val.staff_post){draw_post = '<tr> <td class="first-user-row">Должность: </td> <td class="second-user-row"><div class="rpd-postuser">'+val.staff_post+'</div></td> </tr>';}
			 ...
			 if(val.staff_depart_id){wrDepart = '<tr> <td class="first-user-row">Отдел: </td> <td class="second-user-row"><div class="rpd-dep">'+val.staff_depart_id+'</div></td> </tr>';}
				$('#'+prefix+'start').before('<li class="card-wrapper-staff"> <div class="card-padding">'+waitU+'<div class="overlay-rpd"><div class="brd-usr" <?if($userLevel['oper_correct_staff']=='0'){?>style="width: 30px;"<?}?> data-staffid="'+val.staff_id+'"><?if($userLevel['oper_correct_staff']=='1'){?><a id="staff_trasher" class="red-color" href="javascript:void(0);"><i class="fa fa-trash-o"></i></a> <a id="staff_corusr" href="javascript:void(0);"><i class="fa fa-pencil"></i></a><?}?> <a id="eyetracking" class="green-color" href="javascript:void(0);"><i class="fa fa-eye"></i></a><div style="clear:both"></div></div></div> <div class="left-part-ave"><img src="<?=$pathSite?>/images/comprofiler/'+val.staff_avatar+'"> </div> <div class="right-part-data"> <b class="rpd-name">'+val.staff_lastname+' '+val.staff_name+' '+val.staff_secondname+'</b> <table class="tbls-users" border="0" style="border-collapse:collapse;margin-top: 5px;margin-bottom:5px;" cellpadding="0"> '+draw_post+'<tr> <td class="first-user-row">Компания: </td> <td class="second-user-row"><div class="rpd-comp">'+val.staff_company_id+'</div></td> </tr> '+wrDepart+' <tr> <td class="first-user-row">Локация: </td> <td class="second-user-row"><div class="rpd-loca">'+val.staff_location+'</div></td> </tr> '+wrEmail+''+wrAts+'  </table> </div> </div> <div style="clear:both"></div> </div> </li>');
				wrAts = '';
				waitU = '';
				wrEmail= '';
				wrDepart= '';
